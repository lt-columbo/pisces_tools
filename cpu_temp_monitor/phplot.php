...
// PHPlot for Pisces Dashboard Modified.
// =====================================================================================================
/**
...
class PHPlot {
    ...
    function SetInputFile($which_input_file) {
        ...
        if (!$im) {
            return FALSE;
        }  // GetImage already produced an error message.
...
/////////////////////////////////////////////
     ...
     * or a large one included from the file 'rgb.inc.php' ==> (NOTE: now a method getLargeColorArray()) .
     *
    ...
    function SetRGBArray($which_color_array) {
        if (is_array($which_color_array)) {           // User defined array
            ...
            $this->rgb_array = getLargeColorArray();
        } else {
    ...
    function SetDataColors($which_data = NULL, $which_border = NULL, $alpha = NULL) {
        ...
        if (!isset($alpha)) {
            $alpha = 0;
        } // Actual default is opaque colors.
        ...
        foreach ($colors as $color) {
            ...
            if (!$color_array) {
                return FALSE;
            } // SetRGBColor already did an error message.
...
/////////////////////////////////////////////
    ...
    protected function ProcessText($draw_it, $font_id, $angle, $x, $y, $color, $text, $halign, $valign) {
        ...
        if (is_array($font_id)) {
            $font = $font_id;
        } // Use supplied array; deprecated
        elseif (!empty($font_id) && isset($this->fonts[$font_id])) {
            $font = $this->fonts[$font_id];
        } else {
            $font = $this->fonts['generic'];
        } // Fallback default, or font_id is empty.
...
/////////////////////////////////////////////
    ...
    protected function SetLabelType($mode, $args) {
        ...
        switch ($type) {
            ...
            case 'printf':
                if (isset($args[1])) // Accept 1, 2, or 3 format strings (see FormatLabel)
                {
...
/////////////////////////////////////////////
    ...
    protected function CheckOptionArray($opt, $acc, $func) {
        ...
        foreach ($opt_array as $option) {
            ...
            if (is_null($choice)) {
                return NULL;
            } // In case CheckOption error handler returns
            $result[] = $choice;
        }
        return $result;
    }
...
//////////////////////////////////////////////////////////
    ...
    protected function CalcStep125($range, $min_ticks) {
        ...
        if ($f > 0.69897) {
            $tick_step *= 5;
        }   // Note 0.69897 = log10(5)
        elseif ($f > 0.301) {
            $tick_step *= 2;
        }  // Note 0.30103 = log10(2), truncated to fix edge cases
        return $tick_step;
    }
    ...
    protected function CheckLabels() {
        ...
        } // Not set with SetXDataLabelAngle()
        else {
            $this->x_data_label_angle = $this->x_data_label_angle_u;
        } // Set with SetXDataLabelAngle()
...
/////////////////////////////////////////////
    ...
    protected function GetImageBorderWidth() {
        ...
        if ($this->image_border_type == 'raised') {
            return 2;
        } // Default for raised border is 2 pixels.
    ...
    protected function DrawPlotBorder($draw_axes = TRUE) {
        ...
        if (isset($this->plot_border_type)) {
            ...
            return TRUE;
        } // Default to no border for plots without axes (e.g. pie charts)
...
/////////////////////////////////////////////
    ...
    protected function GetBarColors($row, $idx, &$vars, &$data_color, &$shade_color, &$border_color) {
        ...
        if (empty($vars)) {
            ...
            }  // Plot needs dark colors for shading
            $custom_color = (bool) $this->GetCallback('data_color');
            $num_data_colors = count($this->ndx_data_colors);
    ...
    protected function SetupAreaPlot($stacked, &$xd, &$yd) {
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            for ($idx = 0; $rec < $this->records_per_group; $rec++, $idx++) {
                ...
                if (!$stacked) {
                    $y = 0;
                } // For non-stacked plots, reset Y in each loop.
            }
...
/////////////////////////////////////////////
    ...
    protected function DrawDots($paired = FALSE) {
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            {
                $iv = 0.5 + $row;
            }                       // Place text-data at 0.5, 1.5, 2.5, etc...
            else {
                $iv = $this->data[$row][$rec++];
            }        // Read it, advance record index
    ...
    protected function DrawLines($paired = FALSE) {
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $idx++) {
                ...
                if ($this->datatype_error_bars) {
                    $rec += 2;
                } // Skip over error bar values, even in missing data case.
            }
        }
    ...
    protected function getLargeColorArray() {
        /* --------------------------------------------------------------
    * This was in a separate file, lt-columbo added as a method to
    * the class to reduce changes needed in dashboard
    *
